...
//根据ID填充数据帧的data区域
//===========================================================
static void DTFrameAddData(u8 frame_num,u8 *_cnt)//根据匿名协议给上位机发送数据
{
	s16 temp_data;
	s32 temp_data_32;
	switch(frame_num)
...
static void DTFrameSend(u8 type, u8 id_addr, u8 addr)
{
	...
	CycleSendData[_cnt++]=0;
	//==
	DTFrameAddData(_id,&_cnt);//根据匿名协议发送对应数据
	//==
...
//发送字符串给上位机log区域显示
//===========================================================
void AnoDTSendF1(u8 type, u8 dest_addr, u8 d1)//F1灵活帧 //USE_U2 USE_HID
{
	u8 _cnt = 0;
	
	otherDataTmp[_cnt++] = FRAME_HEAD;//0xAA
	otherDataTmp[_cnt++] = dest_addr;//0xFF
...
u8 DaTatoSend[100];
void ANODT_SendF3(s16 _a,s16 _b,s16 _c)
{
	u8 _cnt=0;
	...
	OtherSendDataAdd(USE_HID|USE_U2,DaTatoSend,_cnt);
	//ANO_DT_LX_Send_Data(DaTatoSend,_cnt );//USE_U2 USE_HID
}
void ANODT_SendF2(s16 _a)
{
	u8 _cnt=0;
	DaTatoSend[_cnt++]=0xAA;
	DaTatoSend[_cnt++]=0xFF;
	DaTatoSend[_cnt++]=0xF2;
	DaTatoSend[_cnt++]=2;
