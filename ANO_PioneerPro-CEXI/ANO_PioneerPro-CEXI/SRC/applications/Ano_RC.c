...
void unlock(u8 dT_ms)
{
	...
	if(flag.unlock_sta == 0)
	{
		...
		{		
			if(flag.unlock_err == 0)
			{
				...
				//
				AnoDTSendStr(USE_HID|USE_U2,SWJ_ADDR,LOG_COLOR_GREEN,"Unlock OK!");//解锁成功信息发送
			...
			else 
			{
				...
				{
					AnoDTSendStr(USE_HID|USE_U2,SWJ_ADDR,LOG_COLOR_RED,"Unlock Fail! IMU异常");//解锁失败信息发送
				}
				else if(flag.unlock_err == 2)
				{
					AnoDTSendStr(USE_HID|USE_U2,SWJ_ADDR,LOG_COLOR_RED,"Unlock Fail! 内气压计异常");
				}
				else if(flag.unlock_err == 3)
				{
					AnoDTSendStr(USE_HID|USE_U2,SWJ_ADDR,LOG_COLOR_RED,"Unlock Fail! 惯性传感器异常");
				}
				else if(flag.unlock_err == 4)
				{
					AnoDTSendStr(USE_HID|USE_U2,SWJ_ADDR,LOG_COLOR_RED,"Power Low,Unlock Fail! 电池电压异常");
				}
				else
				{
	...
	else
	{
		if(flag.unlock_cmd == 0)
		{
			AnoDTSendStr(USE_HID|USE_U2,SWJ_ADDR,LOG_COLOR_GREEN,"FC Output Locked! ");//上锁
		}		
		flag.unlock_sta = flag.unlock_cmd;
	}
