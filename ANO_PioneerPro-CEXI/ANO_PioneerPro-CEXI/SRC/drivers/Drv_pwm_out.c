...
extern u8 xifu_step,tuoluo_step,lock_cmd,unlock_cmd;
u8 xifu=0,tingzhi=0;
u16 PWM_HE=0;
...
void SetPwm ( int16_t pwm[MOTORSNUM] )//输出PWM值
{
	if(tuoluo_step == 1)//tuoluo_step 0时进行解锁 1时进行加速 平均达到1650转换至2进行降速 降至脱离顶板至3
	{
		xifu = 1;
		PWM_HE = pwm[0]+pwm[1]+pwm[2]+pwm[3];
		if(PWM_HE >= 2600)//平均大于1650
		{
			tuoluo_step = 2;
		}
	}
	else if(tuoluo_step == 2)
	{
		PWM_HE=pwm[0]+pwm[1]+pwm[2]+pwm[3];
		if(PWM_HE<=3000)//平均小于1750
		{
			xifu = 0;
		}
		else
	...
	if((xifu_step == 2 || xifu == 1) && tingzhi==0)//加入急停
	{
		TIM1->CCR4 = 6000; //1 4*计算单元+4000 6000对应1500
		TIM1->CCR3 = 6000; //2
		TIM1->CCR2 = 6000; //3
		TIM1->CCR1 = 6000; //4
		if (lock_cmd == 0 && xifu == 0)//解锁状态
		{
			flag.unlock_cmd = 0;
			lock_cmd = 1; //上锁
		}
	...
	{
		TIM1->CCR4 = PWM_RADIO * (get_max(pwm[0],280)) + INIT_DUTY; //1 4*计算单元+4000 pwm[0-7]为上位机*0.1的值
		TIM1->CCR3 = PWM_RADIO * (get_max(pwm[1],280)) + INIT_DUTY; //2 最低速为1280在脱落1过程中
		TIM1->CCR2 = PWM_RADIO * (get_max(pwm[2],280)) + INIT_DUTY; //3
		TIM1->CCR1 = PWM_RADIO * (get_max(pwm[3],280)) + INIT_DUTY; //4		
	}
	else
	{
		//pwm范围0-1000，转换到4000-8000，折合1000-2000us     
		TIM1->CCR4 = PWM_RADIO * (pwm[0]) + INIT_DUTY; //1 4*计算单元+4000 pwm[0-7]为上位机*0.1的值
		TIM1->CCR3 = PWM_RADIO * (pwm[1]) + INIT_DUTY; //2 怠速为1200
