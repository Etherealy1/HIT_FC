...
void GPS_data_analysis(void)
{
	...
	if (Gps_information.new_pos_get)//计算经纬度的相对位移
	{
	...
	ext_sens.fc_gps.st_data.S_NUM = Gps_information.satellite_num;//卫星数
	ext_sens.fc_gps.st_data.LNG = Gps_information.longitude;//经度
	ext_sens.fc_gps.st_data.LAT = Gps_information.latitude;//纬度
	ext_sens.fc_gps.st_data.ALT_GPS = gps_hmsl / 10; //mm->cm//海拔
	ext_sens.fc_gps.st_data.N_SPE = gps_N_vel / 10;	  //mm->cm 南向速度
	ext_sens.fc_gps.st_data.E_SPE = gps_E_vel / 10;	  //mm->cm 东向速度
	ext_sens.fc_gps.st_data.D_SPE = gps_D_vel / 10;	  //mm->cm 地向速度
	//按协议处理赋值
//	u32 tmp;
...
void GPS_Data_Processing_Task(u8 dT_ms)//100hz处理函数
{
	...
	if (Gps_information.satellite_num >= 6   //卫星数量大于6时可进入GPS模式
