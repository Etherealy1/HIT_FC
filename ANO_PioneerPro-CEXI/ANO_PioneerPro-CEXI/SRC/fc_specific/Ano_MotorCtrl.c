...
//	}
	...
	if(flag.unlock_sta)
	{		
		IDLING = 10*LIMIT(Ano_Parame.set.idle_speed_pwm,0,30);//取20*10
	...
	if(flag.motor_preparation == 1)//怠速运转完毕
	{	
...
*/    
		...
		motor_step[m1] = mc.ct_val_thr  +mc.ct_val_yaw -mc.ct_val_rol +mc.ct_val_pit;//yaw偏航 1-4 大小大小
		motor_step[m2] = mc.ct_val_thr  -mc.ct_val_yaw +mc.ct_val_rol +mc.ct_val_pit;//roll翻滚 1-4小大大小
		motor_step[m3] = mc.ct_val_thr  +mc.ct_val_yaw +mc.ct_val_rol -mc.ct_val_pit;//pitch俯仰 1-4大大小小
...
//六轴  sqrt(3)/2 = 0.866f
		...
		{	
			motor_step[i] = LIMIT(motor_step[i],IDLING,1000);//motor_step[i]为中间值取200至1000之间及20%至100%
//			motor_lpf[i] += 0.5f *(motor_step[i] - motor_lpf[i]) ;		
	...
	for(i=0;i<MOTORSNUM;i++)
	{
		if(flag.unlock_sta)
		{
			...
			{
				motor[i] = LIMIT(motor_step[i],IDLING,999);//当flag.unlock_sta==1及解锁状态时 赋值给电机
			}
