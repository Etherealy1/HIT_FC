...
void Att_2level_Ctrl(float dT_s,s16 *CH_N)
{
	...
	if(flag.flight_mode == ATT_STAB)//自稳模式下
	{
	...
	//在此加入att_2l_ct.exp_rol的赋值
	if(cexi_step == 1)
	{
		att_2l_ct.exp_rol= 90;
//		AnoDTSendStr(USE_HID|USE_U2,SWJ_ADDR,LOG_COLOR_GREEN,"JIAODU FUZHI ON......");//\n为回车	开启恒温功能发送信息
	}
	//////////////////////////////////////////////////////////////
	...
	if(att_2l_ct.yaw_err<-180) att_2l_ct.yaw_err += 360;
		...
		att_2l_ct.fb_rol = (imu_data.rol ) ;//当前角度
		att_2l_ct.fb_pit = (imu_data.pit ) ;
			
	
	PID_calculate( dT_s,            //周期（单位：秒）
										0 ,				//前馈值
										att_2l_ct.exp_rol ,				//期望值（设定值）横滚
...
void Att_1level_Ctrl(float dT_s)
{
	...
	ctrl_parameter_change_task();
		...
		/*目标角速度赋值*/
		 for(u8 i = 0;i<3;i++)
		{
			att_1l_ct.exp_angular_velocity[i] = val_2[i].out;//将上一步角度环PID计算所得出的val_2[i].out赋值
		}
	
		/*目标角速度限幅*/  //ROL 0  PIT 1  YAW 2
 ...
 for(u8 i = 0;i<3;i++)
 {
	 ...
	 ct_val[i] = (val_1[i].out);//PID返回值
 }										 
	/*赋值，最终比例调节*/
	mc.ct_val_rol =                   FINAL_P *ct_val[ROL];//FINAL_P 0.35 根据电机KV赋值
